...
function _aniwatchDevPlugin(): Plugin {
  ...
  const animeKaiSearch = async (query: string) => {
    try {
      console.log(`[DEV AnimeKAI] Searching for: "${query}"`);
      const r = await fetch(`${ANIMEKAI_SEARCH_URL}?keyword=${encodeURIComponent(query)}`, { 
        headers: ANIMEKAI_AJAX_HEADERS,
        signal: AbortSignal.timeout(10000)
      });
      if (!r.ok) {
        console.error(`[DEV AnimeKAI] Search failed with status ${r.status}`);
        return [];
      }
      const d = await r.json() as { status: string; result?: { html: string } };
      const html = d.result?.html || '';
      if (!html) {
        console.log(`[DEV AnimeKAI] No results (empty HTML) for "${query}"`);
        return [];
      }
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        try {
          ...
          // Helper for retries (ported from server.js for consistency)
          const withRetry = async (fn: any, { retries = 2, delay = 1000, label = '' } = {}) => {
            let lastErr;
            for (let i = 0; i <= retries; i++) {
              try {
                ...
                lastErr = err;
                if (i < retries) {
                  console.log(`[DEV retry] ${label} attempt ${i+1} failed, retrying in ${delay * (i+1)}ms...`);
          ...
          switch (action) {
            ...
            case 'info': {
              ...
              // Handle jikan:: provider (resolve via AnimeKAI search)
              if (id.startsWith(`jikan${ID_SEPARATOR}`)) {
                try {
                  const malId = id.slice(`jikan${ID_SEPARATOR}`.length);
                  console.log(`[DEV jikan->AnimeKAI] Resolving MAL ID: ${malId}`);
                  
                  const jikanData = await withRetry(async () => {
                    const resp = await fetch(`https://api.jikan.moe/v4/anime/${malId}`, { 
                      headers: { 'Accept': 'application/json', 'User-Agent': 'nyanime-dev/1.0' } 
                    });
                    if (!resp.ok) throw new Error(`Jikan API returned ${resp.status}`);
                    return await resp.json();
                  }, { label: `Jikan Info ${malId}` }) as { data?: { title?: string; title_english?: string } };
                  
                  const animeTitle = jikanData?.data?.title || jikanData?.data?.title_english || '';
                  if (!animeTitle) { 
                    console.error(`[DEV jikan->AnimeKAI] No title found for MAL ID ${malId}`);
                    send(404, { success: false, error: 'Anime not found on Jikan' }); 
                    return; 
                  }
                  
                  const kaiResults = await animeKaiSearch(animeTitle);
                  if (kaiResults.length > 0) {
                    const slug = kaiResults[0].id.split(ID_SEPARATOR)[1];
                    const info = await animeKaiInfo(slug);
                    if (info.aniId) {
                      const episodes = await animeKaiEpisodes(info.aniId);
                      const mappedSub = episodes.filter((ep: any) => ep.hasSub).map((ep: any) => ({
                        number: ep.number, title: `Episode ${ep.number}`,
                        episodeId: `${ANIMEKAI_PROVIDER}${ID_SEPARATOR}${slug}${ID_SEPARATOR}${ep.token}`,
                        isFiller: false,
                      }));
                      const mappedDub = episodes.filter((ep: any) => ep.hasDub).map((ep: any) => ({
                        number: ep.number, title: `Episode ${ep.number}`,
                      ...
                      send(200, { success: true, data: {
                        id: `${ANIMEKAI_PROVIDER}${ID_SEPARATOR}${slug}`,
                        name: info.title || kaiResults[0].name, jname: info.jname || '',
                        poster: '', description: '',
                        stats: { type: 'TV', status: 'Unknown', episodes: { sub: mappedSub.length, dub: mappedDub.length } },
                        genres: [], episodes: { sub: mappedSub, dub: mappedDub },
                        provider: ANIMEKAI_PROVIDER,
                      }});
                      return;
                    }
                  ...
                  // Fallback: If AnimeKAI fails, try to return basic Jikan info 
                  // so the player can at least try Torrent streaming
                  console.warn(`[DEV jikan->AnimeKAI] Failed to map MAL ID ${malId} to AnimeKAI, returning basic info`);
                  send(200, { success: true, data: {
                    id: `jikan${ID_SEPARATOR}${malId}`,
                    name: animeTitle, jname: jikanData?.data?.title_japanese || '',
                    poster: jikanData?.data?.images?.jpg?.large_image_url || '',
                    description: jikanData?.data?.synopsis || '',
                    stats: { type: 'TV', status: 'Unknown', episodes: { sub: jikanData?.data?.episodes || 0, dub: 0 } },
                    genres: [], episodes: { sub: [], dub: [] },
                    provider: 'jikan',
                  }});
                  return;
                } catch (err: any) {
                  console.error('[DEV jikan->AnimeKAI info error]', err?.message);
                  send(502, { success: false, error: 'Failed to resolve jikan ID: ' + err.message });
                  return;
                }
              }
            ...
            case 'episodes': {
              ...
              if (id.startsWith(`jikan${ID_SEPARATOR}`)) {
                try {
                  const malId = id.slice(`jikan${ID_SEPARATOR}`.length);
                  console.log(`[DEV jikan->AnimeKAI episodes] Resolving MAL ID: ${malId}`);
                  ...
                  const jikanData = await withRetry(async () => {
                    ...
                    if (!resp.ok) throw new Error(`Jikan API returned ${resp.status}`);
                    return await resp.json();
                  }, { label: `Jikan Eps ${malId}` }) as { data?: { title?: string; title_english?: string } };

                  const animeTitle = jikanData?.data?.title || jikanData?.data?.title_english || '';
                  if (!animeTitle) { 
                    console.error(`[DEV jikan->AnimeKAI episodes] No title found for MAL ID ${malId}`);
                    send(404, { success: false, error: 'Anime not found on Jikan' }); 
                    return; 
                  }
                  ...
                  if (kaiResults.length > 0) {
                    ...
                    if (info.aniId) {
                      const episodes = await animeKaiEpisodes(info.aniId);
                      const mapped = episodes.filter((ep: any) => ep.hasSub).map((ep: any) => ({
                        number: ep.number, title: `Episode ${ep.number}`,
                        episodeId: `${ANIMEKAI_PROVIDER}${ID_SEPARATOR}${slug}${ID_SEPARATOR}${ep.token}`,
                        isFiller: false,
                      }));
                  ...
                  // Fallback: Return predicted episodes
                  const totalEps = jikanData?.data?.episodes || 12;
                  console.warn(`[DEV jikan->AnimeKAI episodes] Failed to map, returning ${totalEps} predicted episodes`);
                  const predicted = Array.from({ length: totalEps }).map((_, i) => ({
                    number: i + 1, title: `Episode ${i + 1}`,
                    episodeId: `jikan${ID_SEPARATOR}${malId}${ID_SEPARATOR}${i + 1}`,
                    isFiller: false,
                  }));
                  send(200, { success: true, data: { totalEpisodes: totalEps, episodes: predicted, provider: 'jikan' } });
                  return;
                } catch (err: any) {
                  console.error('[DEV jikan->AnimeKAI episodes error]', err?.message);
                  send(502, { success: false, error: 'Failed to resolve jikan ID: ' + err.message });
                  return;
                }
              }
...
function _streamProxyPlugin(): Plugin {
  return {
    name: 'stream-proxy',
    apply: 'serve',
    configureServer(server) {
...
export default defineConfig(({ mode }) => ({
  server: {
    ...
    proxy: {
      ...
      // Consumet API proxy (for anime metadata) - optional fallback
      '/consumet': {
        target: 'https://consumet.nyanime.qzz.io',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/consumet/, ''),
      },
      // Local Torrent Stream proxy
      '/torrent-stream': {
        target: 'http://127.0.0.1:3000',
        changeOrigin: true,
        secure: false,
      },
      // Local Media Stream proxy
      '/stream': {
        target: 'http://127.0.0.1:3000',
        changeOrigin: true,
        secure: false,
