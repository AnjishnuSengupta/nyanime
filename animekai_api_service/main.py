...
async def sources(
    ...
    ep_token = parts[1]
    # Determine if it's dub from the episodeId (not query parameter)
    is_dub = episodeId.endswith(f"{ID_SEPARATOR}dub")
    ...
    try:
        ...
        embed_url_full = source_data.get("embedUrl", "")
        if embed_url_full:
            try:
                embed_host = f"https://{httpx.URL(embed_url_full).host}"
            except:
                pass
        ...
        return {
            ...
            "data": {
                ...
                "embedURL": embed_url_full or None,
                "provider": "animekai",
            },
        }
    except HTTPException:
