...
const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  getProxyUrl,
  onProviderFailed,
  isTorrentMode: _isTorrentMode = false,
  activeSourceIndex,
  onSourceChange
}) => {
  const isMobile = useIsMobile();
  const [currentSourceIndex, setCurrentSourceIndex] = useState(activeSourceIndex ?? 0);
  
  // Keep internal state synced with external prop
  useEffect(() => {
    if (activeSourceIndex !== undefined && activeSourceIndex !== currentSourceIndex) {
  ...
  const availableTracks: AniwatchTrack[] = React.useMemo(() => {
    ...
    return Array.isArray(tracks) ? tracks.filter(t => {
      ...
      return lang && lang !== 'thumbnails' && !url.includes('thumbnails');
    }) : [];
  ...
  // Locally compute if the current source is a torrent to avoid UI bugs when switching
  const currentIsTorrent = React.useMemo(() => {
    const s = sources[currentSourceIndex];
    if (!s) return false;
    return s.url.includes('/torrent-stream') || (s.quality || '').toLowerCase().includes('torrent');
  }, [sources, currentSourceIndex]);
  ...
  const handleSourceError = React.useCallback(() => {
    ...
    if (nextIndex < sortedSources.length) {
      console.log(`[VideoPlayer] Source ${currentSourceIndex} failed. Trying next source ${nextIndex}...`);
      ...
      // If we exhaust sources, request fresh ones from parent
      if (onProviderFailed) {
        console.log('[VideoPlayer] All sources exhausted, triggering provider rotation');
        onProviderFailed();
        return;
      }
  ...
  useEffect(() => {
    ...
    if (isTorrentLoading) {
      ...
      interval = setInterval(() => {
        setTorrentLoadingSeconds(prev => prev + 1);
      }, 1000);

      // Trigger fallback if loading takes > 20 seconds
      timeout = setTimeout(() => {
        console.log('[VideoPlayer] Torrent timeout reached (20s). Switching to next source.');
        handleSourceError();
      }, 20000);
  ...
  }, [isTorrentLoading, handleSourceError]);
  ...
  const getProxiedSubtitleUrl = useCallback((url: string) => {
    if (url.includes('/torrent-subtitle')) return url;
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      const hls = new Hls({
        ...
        manifestLoadingTimeOut: 10000,  // 10s — fail faster to reach embed fallback on CDN blocks
        manifestLoadingMaxRetry: 3,     // Fewer retries — CDN blocks are persistent on datacenter IPs
        manifestLoadingRetryDelay: 1500, // 1.5s between retries — CDN rate limits are IP-based, not time-based
      ...
      hls.on(Hls.Events.ERROR, (_event, data) => {
        ...
        if (data.fatal) {
          ...
          if (data.details === 'manifestLoadError') {
            // Phase 1: Quick retry via startLoad — CDN blocks are usually persistent on datacenter IPs.
            // Only 1 extra retry cycle (vs. previous 2) to fail fast to embed fallback.
            // Each cycle = HLS.js internal retries (manifestLoadingMaxRetry × manifestLoadingRetryDelay)
            if (fatalRecoveryAttempts <= 1) {
              const delay = 1500; // 1.5s — just enough for transient issues
              console.log(`[HLS.js] manifestLoadError — waiting ${delay/1000}s before retry cycle ${fatalRecoveryAttempts}/1`);
            ...
            if (
              ...
              directHls.on(Hls.Events.ERROR, (_ev, d) => {
                if (d.fatal) {
                  ...
                  console.error('[HLS.js] All direct playback methods exhausted');
            ...
            // Phase 3: Try next available source in list (failover to Torrent)
            console.log('[HLS.js] All proxy/direct attempts failed — failing over to next source');
            hls.destroy();
            hlsRef.current = null;
            hlsActiveRef.current = false;
            handleSourceError();
            ...
            // Phase 4: No embed fallback. Nothing left.
            console.error('[HLS.js] All proxy attempts failed and no re-fetch available. Exhausted.');
          ...
          if (fatalRecoveryAttempts > 2) { // Fail faster to next provider (2 instead of 3)
          ...
          if (data.details === 'fragParsingError' && fragParsingFatals > 0) { // Fail faster (0 instead of 1)
  ...
  return (
    <div className="relative w-full bg-black overflow-hidden rounded-xl aspect-video group">
      ...
      {currentIsTorrent && isTorrentLoading && (
        ...
        <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-md transition-opacity duration-300">
          <div className="text-center max-w-xs animate-in zoom-in-95 duration-300">
            <div className="relative mx-auto mb-6">
              <Loader2 className="h-16 w-16 animate-spin text-anime-purple opacity-20" />
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="px-4 py-1.5 bg-anime-purple/20 border border-anime-purple/30 rounded-full">
                <span className="text-anime-purple font-mono font-bold">
          ...
          <video
            ...
            onTimeUpdate={(e) => {
              ...
              if (onTimeUpdate) {
                ...
                onWaiting={() => { if (currentIsTorrent) setIsTorrentLoading(true); }}

                onCanPlay={() => { setIsTorrentLoading(false); }}
                onPlaying={() => { setIsTorrentLoading(false); }}
