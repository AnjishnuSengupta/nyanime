...
const charactersCache = new Map<number, CacheEntry<CharacterData[]>>();
const reviewsCache = new Map<number, CacheEntry<ReviewData[]>>();

const getCached = <T>(cache: Map<number, CacheEntry<T>>, key: number): T | null => {
...
const setCached = <T>(cache: Map<number, CacheEntry<T>>, key: number, data: T) => {
...
export const fetchCharacters = async (animeId: number): Promise<CharacterData[]> => {
  try {
    const cached = getCached(charactersCache, animeId);
    if (cached) return cached;

    const response = await fetch(`${API_BASE_URL}/anime/${animeId}/characters`);
    if (!response.ok) {
      if (response.status === 429) {
        const fallback = getCached(charactersCache, animeId) || generateFallbackCharacters();
        console.warn(`[Jikan] Rate limited while fetching characters for ${animeId}`);
        return fallback;
      }
    ...
    const formatted = data.data.slice(0, 10).map(formatCharacterData);
    setCached(charactersCache, animeId, formatted);
    return formatted;
  } catch (error) {
...
export const fetchReviews = async (animeId: number): Promise<ReviewData[]> => {
  try {
    const cached = getCached(reviewsCache, animeId);
    if (cached) return cached;
    ...
    if (!response.ok) {
      if (response.status === 429) {
        const fallback = getCached(reviewsCache, animeId) || generateFallbackReviews();
        console.warn(`[Jikan] Rate limited while fetching reviews for ${animeId}`);
        return fallback;
      }
      throw new Error(`Server responded with ${response.status}: ${response.statusText}`);
    }
    ...
    const formatted = data.data.slice(0, 5).map(formatReviewData);
    setCached(reviewsCache, animeId, formatted);
    return formatted;
  } catch (error) {
