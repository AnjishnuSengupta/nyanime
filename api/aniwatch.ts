...
function looksPlayableMediaUrl(value: string) {
  ...
  if (lower.includes('.html') || lower.includes('.htm')) {
    return false; // Explicitly reject HTML pages
  }
  return (
    lower.includes('.m3u8') ||
    lower.includes('.mp4') ||
...
function toSourcesShape(payload: unknown, providerName?: string) {
  const data = payload as {
  ...
  if (providerName === 'animepahe') referer = 'https://animepahe.ru/';
  else if (providerName === 'animesaturn') referer = 'https://www.animesaturn.cx/';
  else if (providerName === 'allanime') referer = 'https://allmanga.to/';
  
  return {
    headers: data.headers || { Referer: referer },
    sources,
    tracks,
    subtitles: tracks,
  ...
  try {
    ...
    for (const server of servers) {
      ...
      try {
        ...
        const candidateTracks = (toSourcesShape(candidatePayload, providerName) as { tracks: Array<{ lang: string; url: string }> }).tracks;
  ...
  try {
    switch (action) {
      ...
      case 'sources': {
        ...
        const data = toSourcesShape(payload, usedProvider) as { sources: unknown[]; tracks: Array<{ lang: string; url: string }>; subtitles: Array<{ lang: string; url: string }>; provider?: string };
