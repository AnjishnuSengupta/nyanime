/**
 * Netlify Function: Aniwatch API (Direct scraper via npm package)
...
async function proxyOld(apiPath: string) {
  try {
...
async function handleLegacyPath(p: string) {
  try {
      ...
      for (const server of serversToTry) {
        try {
          ...
          if (srcData?.sources?.length > 0) {
            ...
            // Enrich hd-1/hd-2 with embed URL
...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    ...
    if (legacyPath) return await handleLegacyPath(legacyPath);
    ...
    switch (action) {
      ...
      case 'sources': {
        ...
        // Pre-check which servers are available
        let availableServers: string[] = [];
        try {
          const serverData = await hianime.getEpisodeServers(eid);
    ...
    if (legacy) return await proxyOld(legacy);
