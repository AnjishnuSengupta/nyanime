...
function toHeadersParam(headers?: Record<string, string>): string {
  ...
  try {
    // btoa only supports Latin-1. encodeURIComponent + unescape is a standard way to handle UTF-8.
    const json = JSON.stringify(proxyHeaders);
    return btoa(unescape(encodeURIComponent(json)));
  } catch (e) {
    console.error('[StreamProxy] Failed to encode headers', e);
    return '';
  }
}
...
function toSameOriginProxyUrl(streamUrl: string, headers?: Record<string, string>): string {
  ...
  const baseUrl = import.meta.env.VITE_API_URL || (import.meta.env.DEV ? '' : 'https://backend.nyanime.qzz.io');
