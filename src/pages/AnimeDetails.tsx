...
import { SEO, getAnimeSchema } from '../lib/seo';
    ...
    if (anime) {
      const getEpisodes = async () => {
        ...
        try {
          ...
          const res = await fetch(`${baseUrl}/api/anime/${animeId}/episodes?audio=sub`);
          const data = res.ok ? await res.json() : { episodes: [] };
          const fetchedEpisodes = data.episodes || [];

          // For airing anime with unknown total episodes (e.g. Slime S4, One Piece),
          // anime.episodes is null. In that case, only show actually fetched/aired episodes
          // — don't pad with ghost "Not Released" slots.
          // For completed or announced-total anime, pad to the known total.
          let episodeCount: number;
          if (anime.airing && !anime.episodes) {
            // Show only what has aired — no ghost padding
            episodeCount = fetchedEpisodes.length;
          } else {
            episodeCount = anime.episodes || Math.max(fetchedEpisodes.length, anime.airingEpisodes || 0);
          }

          const totalSlots = Math.max(episodeCount, fetchedEpisodes.length);
          const transformedEpisodes = Array.from({ length: totalSlots }, (_, i) => {
            const epNum = i + 1;
            const existing = fetchedEpisodes.find((e: { number: number; id?: string; episodeId?: string; title?: string }) => e.number === epNum);
            // An episode is released if the provider has it, or if we know the ep number
            // is within the aired count (for anime with a known total where provider lags slightly)
            const released = !!existing || (!!anime.episodes && epNum <= (anime.airingEpisodes || 0));
            return {
              id: existing ? existing.id : `${id}-episode-${epNum}`,
              episodeId: existing ? existing.episodeId : `${id}-episode-${epNum}`,
              number: epNum,
              title: existing?.title || `Episode ${epNum}`,
              isFiller: false,
              released,
            };
          });
          ...
          console.error("Failed to fetch canonical episodes:", err);
          const episodeCount = anime.episodes || 12;
          const transformedEpisodes = Array.from({ length: episodeCount }, (_, i) => ({
            id: `${id}-episode-${i + 1}`,
            episodeId: `${id}-episode-${i + 1}`,
            number: i + 1,
            title: `Episode ${i + 1}`,
            isFiller: false,
            released: i < (anime.airingEpisodes || episodeCount)
          }));
          setEpisodes(transformedEpisodes);
        } finally {
      ...
      // Fetch next episode airing info for currently-airing anime
      if (anime.airing) {
        const fetchNextEpisodeInfo = async () => {
          try {
            const baseUrl = import.meta.env.VITE_API_URL || '';
            const r = await fetch(`${baseUrl}/api/anime/${animeId}/next-episode`);
            if (r.ok) {
            ...
            console.warn('[NextEp] Failed to fetch next episode info:', err);
          }
        };
  ...
  return (
    <div className="min-h-screen bg-anime-darker">
      <SEO
        ...
        description={`Watch ${anime.title} anime online free. ${anime.synopsis || 'High quality anime streaming'}. Genre: ${anime.category}. IMDb Rating: ${anime.rating || 'N/A'}`}
        keywords={[
          anime.title,
          'watch anime',
          `${anime.title} anime`,
            ...
            <div className="glass-card p-6 rounded-xl">
              ...
              {anime.airing && (
                <div className="mb-6 p-3 bg-green-500/10 border border-green-500/20 rounded-lg">
                  ...
                  {!countdown && broadcastSchedule && (
                    <p className="text-white/50 text-xs mt-1 ml-6">
                      Airs {broadcastSchedule}
                    </p>
                  )}
                  {!countdown && !broadcastSchedule && (
                    <p className="text-white/50 text-xs mt-1 ml-6">
